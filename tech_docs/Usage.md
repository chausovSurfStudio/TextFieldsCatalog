...
- [PasteOverflowPolicy](#PasteOverflowPolicy)
...
````
Метод позволяет ограничить максимальное число символов в поле. Поведение при вводе строки больше, чем `maxLength`, настраивается с помощью  [PasteOverflowPolicy](#PasteOverflowPolicy).
...
````swift
func setup(text: String?, animated: Bool = true, ignoreFormatter: Bool = false, validateText: Bool = true)
````
В силу того, что доступ к UITextField напрямую закрыт - данный метод служит для установки текста в поле ввода.
Имеет параметры для игнорирования валидации и форматирования. Также поддерживает возможность анимации.
...
````
Метод позволяет откатить поле ввода до девственного состояния, очистив текст и обновив все внутренние переменные (поддерживает возможность анимации).
...
Дает определить поведение при вставке для поля с ограниченым максимальным количеством символов.

Есть два варианта работы:
* `.noChanges` - если длина нового текста больше  `maxLength`, никаких изменений в поле не происходит
* `.textThatFits` - если длина превышает  `maxLength`, вставляемое значение обрезается до допустимых параметров
