...
* ```var onBeginEditing: ((UnderlinedTextField) -> Void)?``` - вызвается при начале редактирования значения в поле ввода
...
* `.flexible(CGFloat, CGFloat)` - высота будет изменяться в зависимости от наличия ошибки/подсказки/состоянии поля ввода. При этом обязательно указать два значения: минимальную высоту поля ввода, а также минимальное расстояние от лейбла с подсказкой до нижней границы поля
