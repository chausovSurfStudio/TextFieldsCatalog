...
Отдельным особняком в данном каталоге стоит такое поле, как `UnderlinedTextView`. на данный момент это пока единственная обертка на `UITextView`.
...
при написании библиотеки некие инструменты и внутренние классы, используемые в ней, показались достаточно важными и функционально насыщенными, в силу чего было принято решение сделать их открытыми для стороннего использования.
