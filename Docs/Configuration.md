...
Каждый из типов требует в качестве параметра передать объект с конфигурацией, на основе которого будет определяться логика работы плейсхолдера и его внешний вид.
...
И последнее - при начальной конфигурации поля вы обязаны указать тип плейсхолдера. Support плейсхолдер же устанавливается по желанию, для этого есть метод (только у UnderlinedTextField)

````swift
public func configure(supportPlaceholder: String, configuration: NativePlaceholderConfiguration)
````

Рекомендуется при частом его использовании на проекте - сделать обертку-прокси над ним в своем наследнике, метод, который бы принимал текст, добавлял бы объект configuration и пробрасывал бы эти параметры в указанный выше метод.
